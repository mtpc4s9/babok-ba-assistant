...
```mermaid
graph TD
    %% Context Level DFD (Gane-Sarson Draw.io Style)
    %% Note: Multi-line strings use single-line \n representation to guarantee 100% crash-free parsing in Draw.io.
    
    %% Central System Process
    SYS(["1.0\nSchoolsBuddy ASA Management System\n[Core Application Boundary]"])
    ...
    %% Data Flows (Strictly Nouns)
    EE1 -->| "Club Registration Form" | SYS
    SYS -->| "Invoice Summary & QR Code" | EE1
...
```mermaid
graph TD
    %% Level 1 DFD (Gane-Sarson Draw.io Style)
    %% Note: Multi-line strings use single-line \n representation to guarantee 100% crash-free parsing in Draw.io.
    
    %% Processes
    P1(["1.1\nValidate Registration Form\n[SchoolsBuddy Backend]"])
    P2(["1.2\nProcess Online Charge\n[RedDot Gateway Handler]"])
    P3(["1.3\nGenerate Billing Journal\n[NetSuite ERP Integrator]"])
    ...
    P1 -->| "Write Validated Registry" | DS1
    
    P1 -->| "Charge Payload" | P2
    P2 -->| "API Request" | Gateway
    Gateway -->| "API Response" | P2
    P2 -->| "Write Transaction Status" | DS2
    
    DS2 -->| "Read Settled Transactions" | P3
